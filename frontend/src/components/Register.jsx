...
const Register = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema,
    onSubmit: (values) => {
      mutation.mutateAsync(values).catch((e) => console.log(e));
    },
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-gray-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-6 p-8 bg-gray-800 bg-opacity-70 rounded-xl backdrop-blur-sm border border-gray-700 shadow-xl">
        <div className="text-center">
          ...
          >
            Create your account to share ideas
          </p>
        </div>

        {/* Alert messages */}
        {mutation.isPending && (
          <AlertMessage type="loading" message="Creating your account..." />
        )}
        {mutation.isSuccess && (
          <AlertMessage type="success" message="Registration successful! You can now login." />
        )}
        {mutation.isError && (
          <AlertMessage
            type="error"
